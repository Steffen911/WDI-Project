...
public class StationIdFuserLongestString extends AttributeValueFuser<String, Car, Attribute> {

    public StationIdFuserLongestString() {
        super(new LongestString<>());
    }

    @Override
    public boolean hasValue(Car c, Correspondence<Attribute, Matchable> correspondence) {
        return c.hasValue(Car.STATIOND_ID);
    }

    @Override
    public String getValue(Car c, Correspondence<Attribute, Matchable> correspondence) {
        return c.getRegion().getStationId();
    }
    ...
    public void fuse(RecordGroup<Car, Attribute> group, Car fusedRecord, Processable<Correspondence<Attribute, Matchable>> correspondence, Attribute elem) {
        ...
        fusedRecord.setAttributeProvenance(Car.STATIOND_ID, fused.getOriginalIds());
        fusedRecord.getRegion().setStationId(fused.getValue());
    }
