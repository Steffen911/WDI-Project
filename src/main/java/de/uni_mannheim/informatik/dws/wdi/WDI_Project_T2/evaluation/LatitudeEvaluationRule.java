...
public class LatitudeEvaluationRule extends EvaluationRule<Car, Attribute> {
    ...
    public boolean isEqual(Car c1, Car c2, Attribute elem) {
        if (
            c1.getRegion() == null ||
            c2.getRegion() == null ||
            c1.getRegion().getLatitude() == 0 ||
            c2.getRegion().getLatitude() == 0
        ) {
            return false;
        }
        return sim.calculate(c1.getRegion().getLatitude(), c2.getRegion().getLatitude()) > 0.8;
    }
